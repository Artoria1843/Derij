...
const db = mysql.createConnection({
  host: "mysql-1e658162-monkeydkuffi-052f.a.aivencloud.com",
  port: 10352,
  user: "avnadmin",
...
app.post("/productos", verificarAdmin, upload.single("imagen"), (req, res) => {
  const { nombre, precio, Id_categoria, descripcion, stock } = req.body;
  ...
  const sqlProducto = `
    INSERT INTO producto (Nombre, Precio, Id_categoria, Imagen, Descripcion, Stock)
    VALUES (?, ?, ?, ?, ?, ?)
  `;

  db.query(sqlProducto, [nombre, precioNum, categoriaNum, imagen, descripcion || "", stockNum], (err, result) => {
    if (err) return res.status(500).json(err);
...
app.put("/productos/:id", verificarAdmin, upload.single("imagen"), (req, res) => {
  ...
  if (req.file) {
    const imagen = `/uploads/${req.file.filename}`;
    sql = `UPDATE producto SET Nombre=?, Precio=?, Id_categoria=?, Imagen=?, Descripcion=?, Stock=? WHERE Id_producto=?`;
    values = [nombre || "", precio || 0, Id_categoria || null, imagen, descripcion || "", stock || 0, id];
  } else {
    sql = `UPDATE producto SET Nombre=?, Precio=?, Id_categoria=?, Descripcion=?, Stock=? WHERE Id_producto=?`;
    values = [nombre || "", precio || 0, Id_categoria || null, descripcion || "", stock || 0, id];
  }

  db.query(sql, values, (err) => {
    if (err) return res.status(500).json(err);
    db.query("UPDATE inventario SET Stock=? WHERE Id_producto=?", [stock || 0, id], (err2) => {
      if (err2) return res.status(500).json(err2);
...
app.get("/categorias", (req, res) => {
  db.query("SELECT * FROM categoria WHERE Estado='activo'", (err, result) => {
    if (err) return res.status(500).json(err);
    res.json(result);
  });
});
...
app.post("/categorias", verificarAdmin, (req, res) => {
  ...
  db.query("INSERT INTO categoria (Nombre, Estado) VALUES (?, 'activo')", [nombre], (err) => {
    if (err) return res.status(500).json(err);
...
app.delete("/categorias/:id", verificarAdmin, (req, res) => {
  db.query("UPDATE categoria SET Estado='inactivo' WHERE Id_categoria=?", [req.params.id], (err) => {
    if (err) return res.status(500).json(err);
...
app.post("/contacto", (req, res) => {
  ...
  const sql = `INSERT INTO contactos (nombre, email, telefono, asunto, mensaje) VALUES (?, ?, ?, ?, ?)`;
  db.query(sql, [nombre, email, telefono, asunto, mensaje], (err) => {
    if (err) {
      console.log(err);
...
app.get("/ventas", verificarAdmin, (req, res) => {
  const sql = `
    SELECT 
      v.Id_venta, v.Total, v.Estado, v.Estado_envio, v.Fecha,
      u.Nombre AS nombre_usuario, u.Email AS email_usuario, u.Telefono AS telefono_usuario,
      d.Calle, d.Numero, d.Colonia, d.Ciudad, d.Estado AS estado_direccion,
...
app.put("/ventas/:id/entregado", verificarAdmin, (req, res) => {
  db.query("UPDATE venta SET Estado_envio = 'entregado' WHERE Id_venta = ?", [req.params.id], (err) => {
    if (err) return res.status(500).json(err);
...
app.put("/ventas/:id/pagado", verificarAdmin, (req, res) => {
  db.query("UPDATE venta SET Estado = 'pagado' WHERE Id_venta = ?", [req.params.id], (err) => {
    if (err) return res.status(500).json(err);
