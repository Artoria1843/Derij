...
export function Register() {
  ...
  const validations = {
    ...
    email: /^[a-zA-Z0-9._%+-]+@(gmail\.com|hotmail\.com|outlook\.com|icloud\.com|smarcos\.tecnm\.mx|yahoo\.com)$/i,
    phone: /^[0-9]{10}$/,
  ...
  const validateField = (
    ...
    switch (name) {
      case "name":
        ...
        if (!validations.name.test(value)) return "Solo letras y mínimo 3 caracteres";
        return "";

      case "email":
        if (!value) return "Ingresa tu correo";
        if (!validations.email.test(value)) return "Ingresa un correo válido";
        return "";

      case "phone":
        if (!value) return "Ingresa tu teléfono";
        if (!validations.phone.test(value)) return "Debe contener 10 números";
        return "";

      case "password":
        if (!value) return "Ingresa una contraseña";
        if (!validations.password.test(value)) return "8 caracteres, mayúscula, número y símbolo";
        return "";

      case "confirmPassword":
        if (!value) return "Confirma tu contraseña";
        if (value !== updatedForm.password) return "Las contraseñas no coinciden";
        return "";
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (name === "name") value = value.replace(/[^A-Za-zÁÉÍÓÚáéíóúñÑ\s]/g, "");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await axios.post("https://derij.onrender.com/registro", {
        name: formData.name,
      ...
      <div className="w-full max-w-[1000px] flex flex-col md:flex-row rounded-[2rem] overflow-hidden shadow-[0_20px_50px_rgba(0,0,0,0.5)]">
        ...
        <div className="w-full md:w-1/2 relative flex flex-col items-center justify-center p-10 bg-[#5a0209]">
          <div
            ...
            style={{ backgroundImage: "url('https://images.unsplash.com/photo-1652128288793-6a2077ee246b')" }}
          />
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div>
                ...
                <div className="relative mt-1">
                  ...
                  />
                  {formData.name && !errors.name && <CheckCircle className="absolute right-3 top-1/2 -translate-y-1/2 text-green-500 w-5 h-5" />}
                  {errors.name && <AlertCircle className="absolute right-3 top-1/2 -translate-y-1/2 text-red-500 w-5 h-5" />}
                </div>
              ...
              <div>
                ...
                <div className="relative mt-1">
                  ...
                  <input
                    ...
                    onChange={handleChange} placeholder="correo@gmail.com"
                    className={`w-full pl-10 pr-10 py-3 rounded-xl border bg-white outline-none transition ${inputStyle("email")}`}
                  />
                  {formData.email && !errors.email && <CheckCircle className="absolute right-3 top-1/2 -translate-y-1/2 text-green-500 w-5 h-5" />}
                  {errors.email && <AlertCircle className="absolute right-3 top-1/2 -translate-y-1/2 text-red-500 w-5 h-5" />}
                </div>
              ...
              <div>
                ...
                <div className="relative mt-1">
                  ...
                  <input
                    ...
                    maxLength={10} onChange={handleChange} placeholder="10 dígitos"
                    className={`w-full pl-10 pr-10 py-3 rounded-xl border bg-white outline-none transition ${inputStyle("phone")}`}
                  />
                  {formData.phone.length === 10 && !errors.phone && <CheckCircle className="absolute right-3 top-1/2 -translate-y-1/2 text-green-500 w-5 h-5" />}
                  {errors.phone && <AlertCircle className="absolute right-3 top-1/2 -translate-y-1/2 text-red-500 w-5 h-5" />}
                </div>
              ...
              <div>
                ...
                <div className="relative mt-1">
                  ...
                  {formData.password && !errors.password && <CheckCircle className="absolute right-3 top-1/2 -translate-y-1/2 text-green-500 w-5 h-5" />}
                  {errors.password && <AlertCircle className="absolute right-3 top-1/2 -translate-y-1/2 text-red-500 w-5 h-5" />}
                </div>
              ...
              <div>
                ...
                <div className="relative mt-1">
                  ...
                  />
                  {formData.confirmPassword && !errors.confirmPassword && <CheckCircle className="absolute right-3 top-1/2 -translate-y-1/2 text-green-500 w-5 h-5" />}
                  {errors.confirmPassword && <AlertCircle className="absolute right-3 top-1/2 -translate-y-1/2 text-red-500 w-5 h-5" />}
                </div>
